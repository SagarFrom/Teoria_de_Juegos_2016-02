...
bool ParticleShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix,
...
bool ParticleShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateVertexShader(VertexShader, sizeof(VertexShader), NULL, &m_vertexShader);
	...
	result = device->CreatePixelShader(PixelShader, sizeof(PixelShader), NULL, &m_pixelShader);
	...
	HRESULT hr = device->CreateInputLayout(polygonLayout, 3, VertexShader, sizeof(VertexShader), &m_layout);
	/*if (!DXCall())
	Log("Failed creating 2D Input Layout");*/
	DXName(m_layout, "2D Input Layout");
...
bool ParticleShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix,
	const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
	...
	XMMATRIX _worldMatrix = DirectX::XMMatrixTranspose(worldMatrix);
	XMMATRIX _viewMatrix = DirectX::XMMatrixTranspose(viewMatrix);
	XMMATRIX _projectionMatrix = DirectX::XMMatrixTranspose(projectionMatrix);
