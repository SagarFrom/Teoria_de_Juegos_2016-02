...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	m_particleDeviationY = 0.25f;
	m_particleDeviationZ = 2.5f;

	// Set the speed and speed variation of particles.
	m_particleVelocity = 1.3f;
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		positionX = (((float)rand() - (float)rand()) / (RAND_MAX/10)) * m_particleDeviationX;
		positionY = (((float)rand() - (float)rand()) / (RAND_MAX/10)) * (m_particleDeviationY*1.5);
		positionZ = (((float)rand() - (float)rand()) / RAND_MAX/3) * m_particleDeviationZ;

		velocity = m_particleVelocity + (((float)rand() - (float)rand()) / RAND_MAX) * (m_particleVelocityVariation * 0.2);

		red = (((float)rand() - (float)rand()) / RAND_MAX) + 0.8f;
		green = (((float)rand() - (float)rand()) / RAND_MAX) + 0.7f;
